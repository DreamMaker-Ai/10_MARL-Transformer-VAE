    ...
    def test_play(self, current_weights, epsilon):
        ...
        for _ in range(self.env.config.max_episodes_test_play):
            ...
            while not dones['all_dones']:
                ...
                q_logits, scores, z_stats, reconst_maps = \
                    self.policy(self.padded_states, self.mask, training=False)
                # (1,n,action_dim), [(1,num_heads,n,n),(1,num_heads,n,n)],
                # [(1,n,hidden_dim),(1,n,hidden_dim)], (1,n,15,15,6)
...
def main():
    ...
    # Load model  * Comment out the following 3 lines if no load-model
    # load_dir = Path(__file__).parent / 'trial-1/models'  # Comment out if no load model
    # load_name = '/best_model/best_model'  # Comment out if no load model
    # dummy_policy.load_weights(str(load_dir) + load_name)  # Comment out if no load model
