...
class Learner:
    ...
    def update_network(self, minibatchs):
        ...
        for (indices, correction_weights, experiences) in minibatchs:
            ...
            next_masks = np.vstack(next_masks)  # (32,15), bool
            ...
            next_masks = \
                tf.convert_to_tensor(next_masks, dtype=tf.float32)  # (32,15), bool->float32
            next_states_for_q = \
                tf.convert_to_tensor(next_states_for_q, dtype=tf.float32)  # (32,15,15,15,6)

            # Target valueの計算
            next_q_logits, _, _, _ = \
                self.target_q_network(next_states_for_q, masks, training=False)  # (32,15,5)
            ...
            with tf.GradientTape() as tape:
                q_logits, _, z_stats, reconst_maps = \
                    self.q_network(states, masks, training=True)
                # (32,15,5),_,[(32,15,256),(32,15,256)],(32,15,15,15,6)

                """ TD loss """
                actions_one_hot = tf.one_hot(actions, depth=self.action_space_dim)  # (32,15,5)

                Q = q_logits * actions_one_hot  # (32,15,5)
                ...
                reconstruction_losses = \
                    tf.reduce_sum(reconstruction_losses1, axis=-1) / tf.reduce_sum(masks, axis=-1)
                # (32,)
                ...
                # average kl-loss per alive agent
                kl_losses = tf.reduce_sum(kl_losses1, axis=-1) / tf.reduce_sum(masks, axis=-1)
                # (32,)
