...
class Actor:
    ...
    def rollout(self, current_weights):
        ...
        for _ in range(self.env.config.actor_rollout_steps):
            ...
            q_logits, _, _, _ = self.policy(self.padded_states, self.mask, training=False)
            # (1,15,5)
        ...
        if self.env.config.prioritized_replay:
            ...
            states = np.vstack([transition[0] for transition in self.buffer])  # (100,15,15,15,6)
            actions = np.vstack([transition[1] for transition in self.buffer])  # (100,15)
            rewards = np.vstack([transition[2] for transition in self.buffer])  # (100, 15)
            next_states = np.vstack([transition[3] for transition in self.buffer])
            # (100,15,15,15,6)
            dones = np.vstack([transition[4] for transition in self.buffer])  # (100,15)
            mask = np.vstack([transition[5] for transition in self.buffer])  # (100,15)
            next_states_for_q = \
                np.vstack([transition[7] for transition in self.buffer])  # (100,15,15,15,6)

            next_q_values, _, _, _ = \
                self.policy(next_states_for_q, mask, training=False)  # (100,15,5)
            next_actions = tf.argmax(next_q_values, axis=-1)  # (100,15)
            next_actions = tf.cast(next_actions, dtype=tf.int32)
            next_actions_one_hot = tf.one_hot(next_actions, depth=self.action_space_dim)
            # (100,15,5)

            next_maxQ = tf.reduce_sum(next_q_values * next_actions_one_hot, axis=-1)  # (100,15)

            TQ = rewards + self.gamma * (1 - dones) * next_maxQ  # (100,15)

            q_values, _, _, _ = self.policy(states, mask, training=False)  # (100,15,5)
            actions_one_hot = tf.one_hot(actions, depth=self.action_space_dim)  # (10,15,5)
            Q = tf.reduce_sum(q_values * actions_one_hot, axis=-1)  # (100,15)

            td_errors = np.abs(TQ - Q)  # (100,15)

            masked_td_errors = td_errors * mask.astype(np.float32)  # (100,15)
            masked_td_errors = \
                np.sum(masked_td_errors, axis=-1) / \
                np.sum(mask.astype(np.float32), axis=-1)  # (100,)
